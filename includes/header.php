...
<head>
  ...
  <script>
    (function() {
      ...
      // 2. Tab-specific session security (as requested)
      const serverTabId = "<?= $_SESSION['tab_id'] ?? '' ?>";
      const isLoggedIn = <?= isLoggedIn() ? 'true' : 'false' ?>;
      
      if (isLoggedIn && serverTabId) {
        const localTabId = sessionStorage.getItem('vitanova_tab_id');
        const urlParams = new URLSearchParams(window.location.search);
        const isLoginSuccess = urlParams.has('login_success');
        
        if (isLoginSuccess) {
          // This is a fresh login/redirect, authorize this specific tab
          sessionStorage.setItem('vitanova_tab_id', serverTabId);
        } else if (!localTabId || localTabId !== serverTabId) {
          // This tab has no authorization or mismatch (copied link to new tab/browser)
          // We redirect to logout to force re-authentication for security
          window.location.href = "<?= BASE_URL ?>/compte.php?action=logout&reason=new_tab_protection";
        }
      }
    })();
  </script>
